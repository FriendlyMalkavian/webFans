...
class UserManager extends BaseManager {
    constructor(options) {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on(DELETE_USER, (data) => this._deleteUser(data, socket));
    ...
    async _deleteUser({ guid, hash, random } = {}, socket) {
        const user = this.users.get(guid);
        const params = { guid };
        if(user) {
            const isDeleted = await user.delete({ hash, random, params });
            socket ? socket.emit(this.SOCKETS.DELETE_USER, isDeleted) : '';
            return;
        }
        socket ? socket.emit(this.SOCKETS.DELETE_USER, null) : '';
        return null;
    }
